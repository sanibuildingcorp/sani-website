...
(async () => {
  ...
  console.log('\n3. the assistant reads the inbox, whole, and can put an email\'s fact into an estimate\n');
  {
    ...
    ok('THE INBOX IS IN THE PROMPT: every human email as one line, newest first, with who, subject and estimate', /THE INBOX \(info@, every inbound email/.test(sys) && /Dana Ortiz <manager@shorepkwy\.com> \| COI for 3855 Shore Pkwy \| not a customer we know/.test(sys) && /Rafael Mendez <rafa\.m@newmail\.com> \| from my work email \| about SBC-260915-RAFA/.test(sys) && sys.indexOf('DoorDash') === -1, sys.slice(sys.indexOf('THE INBOX'), sys.indexOf('THE INBOX') + 500));
