...
(async function () {
  ...
  }
  {
    ...
    ok('A HUGE LIST IS CUT, NOT SENT WHOLE - the clock is the reason', sent.system.length < 25000 && /list cut here/.test(sent.system), sent.system.length + ' chars');
    sent = null;
  ...
  console.log('\nit reads the generated estimate, line by line\n');
  {
    ...
    ok('a huge estimate is cut, not sent whole', /estimate cut here/.test(sent.system) && sent.system.length < 20000, sent.system.length + ' chars');
