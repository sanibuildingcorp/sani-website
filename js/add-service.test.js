...
(async () => {
  ...
  console.log('\n4. The generator is wired for it\n');
  {
    ...
    ok('THE RESULT IS MERGED, NOT WRITTEN OVER: the ordinary path (analysis, pin, preserved fields, status) runs only when nothing is being added', /if \(addSvc\) \{[\s\S]*?addedReport = mergeAddedService\(record, estimate, \{ text: addSvc\.text, service: addSvc\.service \}\);[\s\S]*?\} else \{[\s\S]*?record\.projectAnalysis = projectAnalysis;[\s\S]*?record\.estimate = estimate;[\s\S]*?record\.status = record\.status === "new" \? "drafted" : record\.status;[\s\S]{0,700}?\n    \}/.test(BG) && /added: addedReport,/.test(BG));
