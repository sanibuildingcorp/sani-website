...
(async () => {
  console.log('\n1. Ask AI: every picture on the job is counted, and the first six are shown\n');
  {
    ...
    ok('the background caller asks for six', /jobPhotos: JOB_PHOTOS, estimateChars: ESTIMATE_CHARS \}\);/.test(fs.readFileSync(path.join(ROOT, 'netlify/functions/assistant-background.js'), 'utf8')) && /const JOB_PHOTOS = 6;/.test(fs.readFileSync(path.join(ROOT, 'netlify/functions/assistant-background.js'), 'utf8')));
