...
{
  /* "I don't like the hero section": the words now sit on ONE photograph -
     the marble bathroom with the tub he chose, on phones and computers
     alike ("same background to the desktop version"). Same rules. */
  const HERO = 'images/hero/home-hero-bath.webp';
  ...
  ok('the page, the preload hint and the CSS variable all point at the same hero',
    (IDX.match(/images\/hero\/home-hero-bath\.webp/g) || []).length >= 3);
  ...
  ok('the hero is still preloaded at high priority — that work was already right',
    /rel="preload" as="image"[^>]*home-hero-bath\.webp[^>]*fetchpriority="high"/.test(IDX));
}
