...
console.log('\n2. Past thirty days, a preview, and the questions phase\n');
{
  ...
  Date.now = realNow;
  ok('PAST 30 DAYS and not approved: "Expired on" and ask for an updated price', /<small>Expired on<\/small>October 23, 2026/.test(old) && /This estimate has passed its 30 days\. Ask us for an updated price before approving\./.test(old));
  ...
  ok('A PREVIEW (never sent): issued today, valid thirty days from today', Math.abs(vu.getTime() - (Date.now() + 30 * 86400000)) < 5000);
