...
console.log('\n2. The generator: both prompts carry the rule, the normalizer trims, the checks stand down\n');
{
  ...
  ok('THE SCOPE WRITER: a repair gets one to four lines and is told to say only what is fixed; the minimum is passed to the enforcement', /\$\{jobSize\.isRepair\(analysis\) \? 1 : MIN_BULLETS\} to \$\{jobSize\.isRepair\(analysis\) \? 4 : MAX_BULLETS\} bullets/.test(SW) && /THIS IS A REPAIR: say only what is fixed and how, in one to four short lines/.test(SW) && /if \(included\.length < \(minBullets \|\| MIN_BULLETS\)\)/.test(SW) && (SW.match(/jobSize\.isRepair\(analysis\) \? 1 : MIN_BULLETS\)/g) || []).length === 2);
}
