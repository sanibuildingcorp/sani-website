...
(async () => {
  ...
  for (const [label, chat] of [['the estimator (inside the estimate)', REC.ref], ['the drawer', 'global']]) {
    ...
    ok(label + ': customer text is "we" and "you", each fact once, Not included = item + who, read back', /Say 'we' and 'you', each fact once; Not included = item \+ who/.test(sys) && /'Countertops - installed by others\.', not 'Countertops are not installed by us\. Your kitchen installers are never asked to install countertops\.'/.test(sys));
