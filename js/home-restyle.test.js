...
ok('THE HERO PHOTO IS THE MARBLE BATHROOM HE CHOSE, on every screen, light and preloaded once (no media split)', /<img class="hero-img" src="images\/hero\/home-hero-bath\.webp\?v=\d+"[^>]*width="657" height="986"[^>]*fetchpriority="high"/.test(IDX) && fs.statSync(path.join(ROOT, 'images/hero/home-hero-bath.webp')).size < 120 * 1024 && (IDX.match(/<link rel="preload" as="image"[^>]*home-hero/g) || []).length === 1 && /<link rel="preload" as="image" href="images\/hero\/home-hero-bath\.webp\?v=\d+" fetchpriority="high">/.test(IDX) && !/<picture>/.test(IDX.slice(IDX.indexOf('class="hero"'), IDX.indexOf('class="hero"') + 4000)));
ok('ON A COMPUTER THE SAME BATHROOM sits on the right at its own sharp size, the words on solid charcoal on the left fading into it', (() => { const m = style.match(/@media\(min-width:900px\)\{\s*html body \.hero \.hero-image\{display:none!important\}\s*html body \.hero \.hero-content\{background:none!important\}\s*html body \.hero \.hero-content::before\{([^\n]*)\}/); return !!m && /width:100vw/.test(m[1]) && /z-index:-1/.test(m[1]) && /rgba\(24,22,20,1\) 54%/.test(m[1]) && /url\('images\/hero\/home-hero-bath\.webp\?v=\d+'\) right center\/auto 112% no-repeat/.test(m[1]) && !/home-hero-room/.test(style); })());
ok('...on a phone the photo sits behind the words themselves, so the tub is not hidden under the light strips, with an overlay light enough to see it', (() => { const m = style.match(/@media\(max-width:899px\)\{\s*html body \.hero \.hero-image\{display:none!important\}\s*html body \.hero \.hero-content\{background:([^\n]*)\}/); return !!m && m[1].indexOf('rgba(24,22,20,.40) 45%') !== -1 && /url\('images\/hero\/home-hero-bath\.webp\?v=\d+'\) center 78%\/cover/.test(m[1]); })());
ok('...never the old stock PNG (it showed TV mounting)', !/hero-img" src="images\/hero\/home-hero\.png/.test(IDX));
ok('the words sit on a dark overlay, white, with the accent in gold', /\.hero \.hero-scrim\{display:block!important;[^}]*rgba\(24,22,20/.test(style) && /\.hero h1\{color:#fff!important/.test(style) && /\.hero h1 em,html body \.hero h1 \.amp\{color:#dcb46a!important/.test(style));
...
ok('THE STAT BOX SITS ON THE LINE between the dark photo and the light strip: half on each', /\.hero \.hero-stats\{position:relative!important;z-index:6!important;background:#2b2926!important;[^}]*margin-bottom:-64px!important\}/.test(style) && /\.hero \.borough-strip\{position:relative!important;z-index:2!important;padding-top:96px!important\}/.test(style) && /\.hero \.hero-content\{overflow:visible!important/.test(style) && /html body \.hero\{overflow:visible!important\}/.test(style));
