...
ok('...on a phone the photo sits behind the words themselves, so the tub is not hidden under the light strips, with an overlay light enough to see it', /@media\(max-width:899px\)\{\s*html body \.hero \.hero-image\{display:none!important\}\s*html body \.hero \.hero-content\{background:linear-gradient\([^)]*rgba\(9,17,29,\.40\) 45%[^)]*\),url\('images\/hero\/home-hero-bath\.webp\?v=\d+'\) center 78%\/cover/.test(style));
