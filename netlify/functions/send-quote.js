...
exports.handler = async function (event) {
  ...
  try {
    ...
    const offQuote = Array.isArray(est.hiddenQuotePhotos) ? est.hiddenQuotePhotos : [];
    const isDoc = (p) => p.kind === "file" || /\.pdf(?:[?#]|$)/i.test(String(p.data)) || /^data:application\/pdf/i.test(String(p.data));
    const photoCount = [...(reqData.photos || []).filter((p) => p && offQuote.indexOf(p.data) === -1), ...(est.quotePhotos || [])].filter((p) => p && p.data && !isDoc(p)).length;
