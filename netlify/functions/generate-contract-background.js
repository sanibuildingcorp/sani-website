// netlify/functions/generate-contract-background.js
// AI Contract Generator — BACKGROUND function (15-min limit, no 10s squeeze).
// Builds a PROJECT-SPECIFIC service agreement from the estimate data and
// saves it to the record. Clients poll get-estimate until record.contract
// appears (or record.contractError on failure).
// POST { ref, force? }
...
exports.handler = async function (event) {
  ...
  try {
    ...
    // Never regenerate after signing; skip if already generated (unless force)
    if (record.contract && record.contract.sections && (record.contract.signed || !force)) {
      return { statusCode: 200, headers: cors(), body: JSON.stringify({ success: true, cached: true }) };
    }

    // Mark as generating: clear stale contract/error so pollers wait for the new one
    record.contract = null;
    delete record.contractError;
    record.contractGeneratingAt = new Date().toISOString();
...
6. Return ONLY the JSON.`;
    ...
    try {
      ...
      record.contractError = "AI returned an invalid contract — tap Generate again";
      await store.setJSON(ref, record);
      return { statusCode: 200, headers: cors(), body: JSON.stringify({ error: record.contractError }) };
    }
    ...
    console.error("generate-contract-background error:", err.message);
    // Best effort: surface the failure to pollers
    try {
      const store2 = getStore({ name: "estimates", siteID: process.env.MY_SITE_ID, token: process.env.MY_BLOBS_TOKEN });
      const rec2 = await store2.get((JSON.parse(event.body || "{}")).ref, { type: "json" });
      if (rec2) { rec2.contractError = err.message; await store2.setJSON((JSON.parse(event.body || "{}")).ref, rec2); }
    } catch (e2) {}
