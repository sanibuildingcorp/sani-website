...
// BACKGROUND version of generate-estimate. Netlify kills normal functions at 10s;
// files named *-background run up to 15 minutes. It returns 202 immediately and the
// dashboard polls get-estimate until aiStatus flips to "done" or "error".
...
- Return ONLY the JSON. No preamble. No code fences.`;
    ...
    // Flag the record so the dashboard's poller knows work has started.
    record.aiStatus = "running";
    record.aiError = "";
    record.aiStartedAt = new Date().toISOString();
    await store.setJSON(ref, record);

    // Call Claude API (may take 20-60s; that is fine in a background function)
    ...
    try {
      ...
      record.aiError = "AI returned invalid JSON";
      await store.setJSON(ref, record);
    ...
    console.error("generate-estimate-background error:", err && err.stack ? err.stack : err);
    try {
      ...
      if (rec2) {
        rec2.aiStatus = "error";
        rec2.aiError = String(err.message || err).slice(0, 300);
        await store2.setJSON(JSON.parse(event.body || "{}").ref, rec2);
      }
    } catch (_) {}
    return { statusCode: 200, headers: cors(), body: JSON.stringify({ ok: false }) };
  }
