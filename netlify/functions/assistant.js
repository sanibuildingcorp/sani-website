...
function systemPrompt(context, screen, memory, insights, inboxText) {
  ...
  return [
    ...
    "ADDITIONAL WORK on an estimate the customer already agreed to (a new service, painting, an extra room, anything not in it): never change, reword or reprice that estimate. Tell him to press ➕ ADD ADDITIONAL WORK on it: that makes a separate estimate linked to it, for the new work only, priced and sent on its own while the agreed one stays exactly as it is. On an estimate that IS additional work (the job says so), price and describe only the additional work.",
