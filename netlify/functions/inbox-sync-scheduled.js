...
//
// THE FULL READ, NOT THE QUICK ONE (Sep 26 2026). inbox-sync has 10 seconds
// and spent them loading the estimate list before reading mail; the
// assistant's inbox froze on Sep 23 and missed the 409 Suydam thread. This
// now starts inbox-sync-background (minutes, contractor-only, so it passes
// the key from env). Without DASHBOARD_KEY it falls back to the quick sync.
//
...
const CALL_MS = 25000; /* the scheduled limit is 30s; the background answers 202 at once */
...
exports.handler = async function (event) {
  ...
  const target = key ? "/.netlify/functions/inbox-sync-background" : "/.netlify/functions/inbox-sync";
  const headers = key ? { "Content-Type": "application/json", "x-sbc-key": key } : { "Content-Type": "application/json" };
  const ctrl = typeof AbortController === "function" ? new AbortController() : null;
  const timer = ctrl ? setTimeout(function () { ctrl.abort(); }, CALL_MS) : null;
  try {
    const r = await fetch(base + target, { method: "POST", headers: headers, body: "{}", signal: ctrl ? ctrl.signal : undefined });
    const text = await r.text();
    return { statusCode: 200, headers: JSON_HEADERS, body: JSON.stringify({ ok: r.ok, status: r.status, ran: target, sync: text.slice(0, 2000) }) };
  } catch (e) {
