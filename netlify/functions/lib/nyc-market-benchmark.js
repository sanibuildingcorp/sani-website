// Sani Building Corp — NYC market benchmark guardrails v2.2
...
const VERSION = 'nyc-benchmark-v2.2';
...
const BENCHMARKS = {
  ...
  demo_bathroom: { label:'Bathroom tearout', unit:'project', low:800, high:2000, confidence:'HIGH', source:'NYC pricing benchmark' },
...
// Direct public Brooklyn competitor pricing, normalized to broad bands.
// These bands provide market context only; they do NOT determine audit status.
...
const ACCESS = {
  ...
  borough: {
    Manhattan:{ low:1.15, high:1.30 }, Brooklyn:{ low:1.05, high:1.15 }, Queens:{ low:1.00, high:1.00 },
    Bronx:{ low:0.95, high:1.00 }, 'Staten Island':{ low:0.93, high:1.00 }
  }
};
...
function totals(estimate){
  const labor=money(sum(estimate&&estimate.labor)), materials=money(sum(estimate&&estimate.materials));
...
function evidence(input,analysis){
  return norm([
    input&&input.customer&&input.customer.address,input&&input.request&&input.request.service,
    input&&input.request&&input.request.selectedServices,input&&input.request&&input.request.description,
    input&&input.request&&input.request.extraNotes,input&&input.contractor&&input.contractor.extraRequest,
    input&&input.request&&input.request.groupedAnswers,analysis&&analysis.project_type,analysis&&analysis.project_summary,
    JSON.stringify(analysis&&analysis.selected_trades||[]),JSON.stringify(analysis&&analysis.site_conditions||{}),
    JSON.stringify(analysis&&analysis.quantities||{}),JSON.stringify(analysis&&analysis.confirmed_scope||[])
  ].filter(Boolean).join(' | '));
}
...
function detectBorough(e){
  if(/\b100\d{2}\b|\b101\d{2}\b|\b102\d{2}\b|\bmanhattan\b/.test(e)) return 'Manhattan';
  if(/\b112\d{2}\b|\bbrooklyn\b/.test(e)) return 'Brooklyn';
  if(/\b111\d{2}\b|\b113\d{2}\b|\b114\d{2}\b|\bqueens\b|\bastoria\b|\blong island city\b|\bflushing\b/.test(e)) return 'Queens';
  if(/\b104\d{2}\b|\bbronx\b/.test(e)) return 'Bronx';
  if(/\b103\d{2}\b|\bstaten island\b/.test(e)) return 'Staten Island';
  return 'NYC';
}

function detectQuantities(e,analysis){
  const q=analysis&&analysis.quantities||{};
  const sqftPattern=([word])=>new RegExp(`${word}[^\\d]{0,45}([\\d,]+(?:\\.\\d+)?)\\s*(?:sf|sq\\.?\\s*ft|square feet)`,'i');
  return {
    paintingSf:firstQ(q,['painting_sf','paintingSf','paintable_sf','paintableSf'])||findNumber(e,[/([\d,]+(?:\.\d+)?)\s*(?:sf|sq\.?\s*ft|square feet)[^|\n]{0,45}(?:paint|paintable)/i,sqftPattern(['paint'])]),
...
function position(value,low,high){
  ...
  const mid=(low+high)/2, ratio=value/mid;
  if(value<low*0.65) return {status:'LOW',ratioToMid:money(ratio)};
  if(value>high*1.35) return {status:'HIGH',ratioToMid:money(ratio)};
  if(value<low||value>high) return {status:'REVIEW',ratioToMid:money(ratio)};
  return {status:'PASS',ratioToMid:money(ratio)};
}
function addCheck(checks,code,b,actual,note,type='MARKET'){
  if(!b||!(actual>0)) return;
  const p=position(actual,b.low,b.high);
  checks.push({code,type,label:b.label,actual:money(actual),expectedLow:money(b.low),expectedHigh:money(b.high),unit:b.unit,confidence:b.confidence||'MED',source:b.source||'',status:p.status,ratioToMid:p.ratioToMid,note:note||''});
}
function addMinimum(checks,code,label,actual,min){
  if(!(actual>0)||!(min>0)) return;
  checks.push({code,type:'MINIMUM',label,actual:money(actual),expectedLow:money(min),expectedHigh:money(min),unit:'project minimum',confidence:'HIGH',source:'Sani internal minimum from Aug 2026 benchmark plan',status:actual<min?'LOW':'PASS',ratioToMid:money(actual/min),note:'Internal mobilization/minimum check; does not auto-change price.'});
}

function singleProject(e,kind){
  const majors=['bathroom','kitchen','painting','flooring','window','water damage','handyman','masonry'];
  const present=majors.filter(k=>new RegExp(k==='window'?'window':k,'i').test(e));
  return present.includes(kind)&&present.filter(x=>x!==kind).length===0;
}

function competitiveContext(e){
  if(!/bathroom/.test(e)) return [];
  const gut=/\bgut\b|demo(?:lition)?\s+to\s+stud|complete bathroom demolition|full bathroom demolition/.test(e);
  const cosmetic=/reglaz|wall panel|cosmetic|refresh|fixture swap|no demolition|without demolition/.test(e);
  const b=gut?COMPETITOR_BANDS.bathroom_gut:cosmetic?COMPETITOR_BANDS.bathroom_refresh:COMPETITOR_BANDS.bathroom_midrange;
  return [{code:'BROOKLYN_COMPETITOR_CONTEXT',label:b.label,low:b.low,high:b.high,source:b.source,informationalOnly:true}];
}

function marketAudit(estimate,analysis,input){
  const e=evidence(input||{},analysis||{}), t=totals(estimate||{}), q=detectQuantities(e,analysis||{}), borough=detectBorough(e);
  const checks=[], structureChecks=[], flags=[];
  const bathroom=sectionSellingTotal(estimate||{},'Bathroom')||sellingTotalBy(estimate,/bath|shower|vanity|toilet|waterproof|cement board|tile|grout|thinset|faucet|plumb/);
  const painting=sectionSellingTotal(estimate||{},'Painting')||sellingTotalBy(estimate,/paint|primer|spackle/);
  const flooring=sectionSellingTotal(estimate||{},'Flooring')||sellingTotalBy(estimate,/engineered hardwood|solid hardwood|flooring install|floor refin|lvp|lvt|vinyl plank|carpet/);
  const windows=sectionSellingTotal(estimate||{},'Windows')||sellingTotalBy(estimate,/window|glazier/);

  const singleBath=singleProject(e,'bathroom');
  const gutBath=/\bgut\b|demo(?:lition)?\s+to\s+stud|complete bathroom demolition|full bathroom demolition/.test(e);
  if(singleBath&&bathroom>0) addCheck(checks,gutBath?'BATH_GUT_PROJECT':'BATH_REFRESH_PROJECT',gutBath?BENCHMARKS.bathroom_gut_standard:BENCHMARKS.bathroom_refresh,bathroom,'Project-level benchmark; competitor context is shown separately.');

  if(q.paintingSf>0&&painting>0){
    const per=painting/q.paintingSf;
    const heavy=/heavy prep|level 5|significant patch|extensive repair|premium paint/.test(e);
    const b=heavy?BENCHMARKS.painting_heavy_prep:borough==='Manhattan'?BENCHMARKS.painting_manhattan:borough==='Brooklyn'?BENCHMARKS.painting_brooklyn:borough==='Queens'?BENCHMARKS.painting_queens:BENCHMARKS.painting_full_scope;
    addCheck(checks,'PAINTING_PER_SF',b,per,`Detected paintable area: ${q.paintingSf} SF.`);
  }

  if(q.flooringSf>0&&flooring>0){
    const per=flooring/q.flooringSf;
    const b=/refinish|sand\s*(?:and|&|\+)\s*(?:coat|finish)/.test(e)?BENCHMARKS.floor_refinishing:/lvp|lvt|vinyl plank/.test(e)?BENCHMARKS.lvp_flooring:/solid hardwood/.test(e)?BENCHMARKS.solid_flooring:BENCHMARKS.engineered_flooring;
    addCheck(checks,'FLOORING_PER_SF',b,per,`Detected flooring area: ${q.flooringSf} SF.`);
    addMinimum(checks,'FLOORING_MINIMUM',MINIMUMS.flooring.label,flooring,MINIMUMS.flooring.amount);
  }

  if(q.tileSf>0){
    const tileLabor=laborSellingBy(estimate,/tile setter|tile install|large[- ]?format|herringbone|mosaic|diagonal/);
    if(tileLabor>0){
      const premium=/24\s*[x×]\s*48|large.?format|herringbone|diagonal|mosaic/.test(e);
      addCheck(checks,'TILE_LABOR_PER_SF',premium?BENCHMARKS.tile_premium_labor:BENCHMARKS.tile_typical_labor,tileLabor/q.tileSf,`Detected tile area: ${q.tileSf} SF.`);
      addMinimum(checks,'TILE_MINIMUM',MINIMUMS.tile.label,tileLabor,MINIMUMS.tile.amount);
    }
  }

  if(q.drywallSf>0){
    const dry=sellingTotalBy(estimate,/drywall|sheetrock|gypsum|taping|mudding|level 4|level 5/);
    if(dry>0){
      const b=/level 5/.test(e)?BENCHMARKS.drywall_level5:/level 4/.test(e)?BENCHMARKS.drywall_level4:/moisture|mold resistant|greenboard/.test(e)?BENCHMARKS.drywall_moisture:BENCHMARKS.drywall_standard;
      addCheck(checks,'DRYWALL_PER_SF',b,dry/q.drywallSf,`Detected drywall area: ${q.drywallSf} SF.`);
    }
  }

  if(q.windowCount>0&&windows>0) addCheck(checks,'WINDOW_PER_UNIT',/custom|specialty|acoustic/.test(e)?BENCHMARKS.window_custom:BENCHMARKS.window_average,windows/q.windowCount,`Detected window count: ${q.windowCount}.`);

  if(q.demoSf>0){
    const demo=sellingTotalBy(estimate,/demolition|tearout|tear out|interior gut|debris removal/);
    if(demo>0) addCheck(checks,'DEMO_PER_SF',BENCHMARKS.demo_full_interior,demo/q.demoSf,`Detected demolition area: ${q.demoSf} SF.`);
  }
  ...
  if(q.waterDamageSf>0){
    const wd=sellingTotalBy(estimate,/water damage|remediation|extraction|structural drying|dry[- ]?out/);
    if(wd>0){ const b=/cat(?:egory)?\s*3|black water|sewage/.test(e)?BENCHMARKS.water_damage_cat3:/cat(?:egory)?\s*2|gray water|grey water/.test(e)?BENCHMARKS.water_damage_cat2:BENCHMARKS.water_damage_cat1; addCheck(checks,'WATER_DAMAGE_PER_SF',b,wd/q.waterDamageSf,`Detected affected area: ${q.waterDamageSf} SF.`); }
  } else if(singleProject(e,'water damage')&&t.grandTotal>0) addCheck(checks,'WATER_DAMAGE_PROJECT',BENCHMARKS.water_damage_project,t.grandTotal,'Project-level water-damage benchmark.');

  if(singleProject(e,'kitchen')&&t.grandTotal>0){
    const kitchenSf=findNumber(e,[/(\d+(?:\.\d+)?)\s*(?:sf|sq\.?\s*ft|square feet)[^|\n]{0,40}kitchen/i,/kitchen[^\d]{0,40}(\d+(?:\.\d+)?)\s*(?:sf|sq\.?\s*ft|square feet)/i]);
    addCheck(checks,'KITCHEN_PROJECT',kitchenSf&&kitchenSf<=90?BENCHMARKS.kitchen_small:BENCHMARKS.kitchen_medium,t.grandTotal,kitchenSf?`Detected kitchen area: ${kitchenSf} SF.`:'Kitchen size not reliably detected; medium benchmark used.');
  }

  if(q.masonrySf>0){
    const mas=sellingTotalBy(estimate,/repoint|pointing|brick|masonry|facade|stucco|parging/);
    if(mas>0){ const b=/stucco|parging/.test(e)?BENCHMARKS.stucco_repair:/spot|localized/.test(e)?BENCHMARKS.repoint_spot:BENCHMARKS.repoint_facade; addCheck(checks,'MASONRY_PER_SF',b,mas/q.masonrySf,`Detected masonry area: ${q.masonrySf} SF.`); addMinimum(checks,'MASONRY_MINIMUM',MINIMUMS.masonry.label,mas,MINIMUMS.masonry.amount); }
  }

  if(singleProject(e,'handyman')&&t.grandTotal>0){ addMinimum(checks,'HANDYMAN_MINIMUM',MINIMUMS.handyman.label,t.grandTotal,MINIMUMS.handyman.amount); }

  const customerSupplied=/customer supplies|customer supplied|owner supplied|client supplied|customer will supply|provided by customer/.test(e);
  if(customerSupplied&&t.subtotal>0){
    const materialShare=t.materials/t.subtotal;
  ...
  const coordination=sum(allLines(estimate).filter(l=>/project coordination|project management|general conditions|supervision/.test(lineText(l))));
  if(t.subtotal>0&&coordination/t.subtotal>0.12) structureChecks.push({severity:'REVIEW',code:'COORDINATION_SHARE_HIGH',message:'Separate coordination/general-conditions lines exceed 12% of direct cost. Verify they are not duplicating markup/overhead.'});
  ...
  const severe=checks.filter(c=>['HIGH','LOW'].includes(c.status));
  const review=checks.filter(c=>c.status==='REVIEW');
  const highConfidenceSevere=severe.filter(c=>c.confidence==='HIGH');
  let status='PASS';
  if(highConfidenceSevere.length||severe.length>=2||structureChecks.some(x=>x.severity==='BLOCK')) status='BLOCK';
  else if(severe.length||review.length||structureChecks.length) status='REVIEW';
  else if(!checks.length) status='NO_BENCHMARK';

  const projectCheck=checks.find(c=>/PROJECT$/.test(c.code)||/^BATH_/.test(c.code));
  const marketPosition=projectCheck?(projectCheck.status==='HIGH'?'HIGH':projectCheck.status==='LOW'?'LOW':projectCheck.status==='PASS'?'COMPETITIVE':'EDGE'):(status==='PASS'?'COMPETITIVE':status==='NO_BENCHMARK'?'UNKNOWN':'REVIEW');

  return {
    version:VERSION,status,marketPosition,pricingChanged:false,customerVisible:false,borough,totals:t,checks,structureChecks,accessFlags:flags,
    competitiveContext:competitiveContext(e),
    calibrationPriority:['Sani completed-job production history','current supplier/subcontractor quotes','NYC published benchmarks','public competitor context','AI judgment'],
    guidance:status==='BLOCK'?'Do not auto-send. Review production rates, quantities, duplicated scope, customer-supplied finish treatment, access and allowances.'
      :status==='REVIEW'?'Estimate is near/outside one or more market or structure guardrails. Contractor review recommended before sending.'
      :status==='PASS'?'Generated pricing is within the available NYC benchmark checks.'
      :'No reliable benchmark match was detected. Use normal estimator review and collect actual-job calibration data.',
