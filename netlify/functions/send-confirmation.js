...
// see SEO-LOG.md Aug 2 entry). Instead of depending on their trigger, the form
// pages (contact.html + index.html) now call THIS function directly after a
// successful Netlify Forms POST — the same client->function pattern the
// handyman flow uses in production every day.
//
// ACCEPTS: POST with JSON body {email, name, service, area, address, phone,
// details, form}. Sends the branded confirmation from contact@ (BCC owner),
// logs to Supabase lead_messages for the dashboard timeline.
//
// DIAGNOSTICS (browser): ?ping=1 for status · ?test=1&email=YOU@X.COM for a live send.
// NOTE: submission-created.js remains deployed as a LOG-ONLY breadcrumb writer,
// so if Netlify's trigger ever revives, customers will NOT get double emails.
//
// v3 changes vs v2:
...
exports.handler = async function (event) {
  ...
  // ── NORMAL PATH: called directly by the website after a form submission ──
  if (event.httpMethod !== "POST") {
    return json(405, { error: "POST a JSON body, or use ?ping=1 / ?test=1&email=..." });
  }
  let data;
  try {
    data = JSON.parse(event.body || "{}") || {};
  } catch {
    return json(400, { error: "unparseable JSON body" });
  }

  const formName = String(data.form || "form");
